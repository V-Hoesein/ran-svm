...
class TextCleaner:
    def __init__(self):
        print("===== Preprocessing =====")
        # dataset custom stopwords read file custom_terms.text
        # custom_stopwords adalah daftar kata yang tidak diinginkan
        custom_stopwords_path = os.path.join(os.path.dirname(__file__), "custom_terms.txt")
        self.custom_stopwords = open(custom_stopwords_path, "r").read().split("\n")
        self.sastrawi_stemmer = StemmerFactory().create_stemmer()
        self.porter_stemmer = PorterStemmer()
        stopword_factory = StopWordRemoverFactory()
        self.combined_stopwords = (
            set(stopword_factory.get_stop_words())
            .union(set(stopwords.words("indonesian")))
            .union(set(self.custom_stopwords))
        )
    ...
    def preprocess_text(self, text) -> str:
        ...
        # Kombinasi stemming Sastrawi dan Porter
        stemmed_words = []
        for word in tokens:
            if word.isalpha():
                stemmed_word = self.sastrawi_stemmer.stem(word)
                # Jika hasil stemming Sastrawi masih panjang, coba Porter Stemmer
                if len(stemmed_word) > len(word) / 2:
...
class TFIDFCalculator:
    ...
    def calculate_tf(self):
        ...
        for doc in self.documents:
            ...
            tf_norm = {
                term: round(count / total_terms, 4) for term, count in tf_raw.items()
            }
...
def generate_csv_manual():
    ...
    dataset_path = os.path.realpath(
        os.path.join(
            os.path.dirname(__file__), "..", "static", "uploads", "dataset.csv"
        )
    )
    result_path = os.path.realpath(
        os.path.join(os.path.dirname(__file__), "..", "static", "uploads", "tfidf.csv")
    )
...
def train_model():
    ...
    model_path = os.path.realpath(
        os.path.join(os.path.dirname(__file__), "..", "static", "uploads", "model.pkl")
    )
    X_train_path = os.path.realpath(
        os.path.join(
            os.path.dirname(__file__), "..", "static", "uploads", "X_train.pkl"
        )
    )
    encoder_path = os.path.realpath(
        os.path.join(
            os.path.dirname(__file__), "..", "static", "uploads", "encoder.pkl"
        )
    )
...
def print_hyperplane(model):
    ...
    if (
        w.shape[1] == 2
    ):  # Jika hanya dua fitur (2D), kita bisa cetak atau visualisasikan hyperplane
...
# generate_csv_manual() // !netral
# train_model()
# print(predict('Ka mau nanya, bisa ka habis pake skinker lagsung pake cream kelly?'))
